...
trait FunSets extends FunSetsInterface:
  ...
  def forall(s: FunSet, p: Int => Boolean): Boolean =
    ...
    def iter(a: Int): Boolean =
      if a>bound then
        true // base case, stop checking
      else if s(a) && !p(a) then // an example of a that satisfies s but not p
        false
      else
        iter(a+1)
    iter(-bound)
   ...
   * that satisfies `p`.
   */
  // if its false that all elements fail p, then at least one element
  // must satisfy p
