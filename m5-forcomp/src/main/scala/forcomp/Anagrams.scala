...
object Anagrams extends AnagramsInterface:
   ...
   *
   */
  // we use lazy val because it might be expensive, we only want to compute
  // once needed
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val xMap = x.toMap
    val updatedMap =
      y.foldLeft(xMap) { (acc, pair) =>
        val (char, yCount) = pair
        val newCount = acc(char) - yCount
        if newCount > 0 then acc.updated(char, newCount)
        else acc - char
      }
    updatedMap.toList.sorted
    ...
    def inner(occurrences: Occurrences): List[Sentence] =
      if occurrences.isEmpty then List(Nil)
          ...
          for
            ...
            rest <- inner(subtract(occurrences, combo))
          yield word :: rest
